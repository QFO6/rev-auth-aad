...
func (c *User) SaveUser() error {
	c.GetAvatar()
	// orList := []interface{}{}
	// orList = append(orList, bson.M{"Mail": c.Mail})
	// orList = append(orList, bson.M{"Identity": c.Identity})
	//check if user exist
	u := new(User)
	q := bson.M{"Identity": c.Identity}
	// q := bson.M{"$or": orList}
	udo := revmongo.NewDo(revmongo.DBName, u)
	udo.Operator = "SYS"
	udo.Query = q
	err := udo.GetByQ()
	...
	cdo := revmongo.NewDo(revmongo.DBName, c)
	cdo.Operator = "SYS"
